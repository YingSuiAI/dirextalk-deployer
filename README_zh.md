...
仅有一类 pre-updater 旧节点可通过显式固定的 d1 契约接管。先设置
`DIREXTALK_LEGACY_ADOPT_SOURCE_DIR=/root/dirextalk/dirextalk-message-server`
和 `DIREXTALK_LEGACY_ADOPT_SSH_USER=root`，执行
`scripts/adopt-legacy-node.sh --dry-run <state.json>`。确认只读输出中的
v0.15.2 镜像/digest、健康、Compose 与宿主 Caddy 证据后，按提示设置精确
确认值并去掉 `--dry-run` 重跑。该命令不会拉取或启动不同镜像；它只生成
updater 使用的不可变 Compose 视图、复制在线 P2P 状态、安装固定 updater，
并向已验证的宿主 Caddy 添加 public jobs 路由。其他旧拓扑或已有正式
Release 状态一律拒绝。
