...
**Architecture:** Add a localhost Streamable HTTP daemon to `dirextalk-mcp`, then connect Codex, Cursor, OpenClaw, Hermes, and generic stdio snippets through a stdio proxy.
...
**Files:**
- Create: `../dirextalk-mcp/src/http-server.ts`
- Test: `../dirextalk-mcp/test/http-server.test.ts`

- [x] Write a failing test that connects with `StreamableHTTPClientTransport` and lists tools.
- [x] Implement `startDirextalkMcpHttpServer`.
- [x] Verify `npm test -- http-server`.

### Task 2: Add MCP Daemon CLI

**Files:**
- Create: `../dirextalk-mcp/src/daemon.ts`
- Create: `../dirextalk-mcp/src/proxy.ts`
- Modify: `../dirextalk-mcp/src/index.ts`
- Test: `../dirextalk-mcp/test/daemon.test.ts`
...
- Modify: `../dirextalk-mcp/README.md`, `../dirextalk-mcp/README_zh.md`
