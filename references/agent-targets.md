...
S6 writes one MCP artifact set under `~/.direxio/nodes/<service_id>/mcp/`, selected from the detected runtime. Codex gets `codex.toml`, Cursor gets `cursor.mcp.json`, OpenClaw gets `openclaw.md` plus `openclaw-server.json`, Hermes gets `hermes.mcp.json`, and other MCP-capable supported runtimes get `mcp-servers.json`. These artifacts use the service-scoped `direxio-mcp` from `direxio-mcp@latest` by default and point to the service credential file through `DIREXIO_CREDENTIALS_FILE`. Generated client snippets launch `direxio-mcp` directly over stdio; the service-scoped local daemon is an optional HTTP proxy endpoint for clients that explicitly need it.
...
- `DIREXIO_AGENT_INSTALL=auto` (default): refresh `direxio-connent@latest` under `~/.direxio/nodes/<service_id>/direxio-connect` and `direxio-mcp@latest` under `~/.direxio/nodes/<service_id>/mcp`, unless explicit binary/command overrides are set. S6 writes short wrappers at `direxio-connect/direxio-connect(.cmd)` and `mcp/direxio-mcp(.cmd)`, installs or refreshes the service-scoped `direxio-connect daemon install --config ~/.direxio/nodes/<service_id>/direxio-connect/config.toml --service-name <service_id> --force`, then attempts the optional `direxio-mcp daemon install --service-name <service_id> --credentials-file ~/.direxio/nodes/<service_id>/credentials.json --host 127.0.0.1 --port 19757`. S6 records direxio-connect as installed only after `direxio-connect daemon status --service-name <service_id>` reports `Status: Running` and recent daemon logs show `direxio-connect is running`. Logs that show agent CLI missing, login/auth/trust failures, ACP startup failures, or agent offline state make S6 fail with `connect_install_status=install_failed`; deploy does not continue to completion until the daemon startup is verified. MCP records `mcp_install_status=installed` when the service-scoped command is available and `mcp_daemon_install_status=installed` only when optional daemon install succeeds.
