...
- **dirextalk-updater**: 独立 GitHub 仓库/Release 的 linux/amd64 binary；production split 主机要求 Ubuntu 24.04+、systemd >= 254。deployer 固定 version/commit/SHA-256，宿主下载校验后作为 root-owned systemd service 安装。fresh deployment 写入 `watchdog_enabled=false`：保留 root-owned control plane 和显式 update/recovery job，但不启动常驻 Docker-event/轮询修复 watchdog。它只拥有 mutation readiness、任务、受保护主机变更、验证、回滚和恢复，不再作为 Message Server 或 Agent 当前版本的展示来源，也不做中央 release discovery。它独立于 Compose；Caddy 只读挂其 socket 目录，不接触 control token，也不安装每日 GitHub discovery timer。
