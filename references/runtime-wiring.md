...
- Cursor wiring uses Cursor Agent CLI, not Cursor Desktop CLI. On POSIX hosts S6 resolves the official `agent` command to its absolute executable path; on Windows it looks for `%LOCALAPPDATA%\cursor-agent\agent.cmd` and writes that as `cmd`. Set `DIREXTALK_CURSOR_AGENT_COMMAND`, `DIREXTALK_CURSOR_COMMAND`, `DIREXTALK_CONNECT_AGENT_CMD`, `DIREXTALK_CURSOR_MODE`, or `DIREXTALK_CONNECT_AGENT_OPTIONS_TOML` to override defaults. If the agent is not logged in, run the resolved command with `login` once and rerun the deployer; S6 will refresh config and reinstall the daemon.
