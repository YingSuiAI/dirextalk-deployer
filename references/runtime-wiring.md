...
- Windows Cursor wiring avoids spawning `cursor.cmd` directly. When S6 can find the Cursor CLI install tree, it writes the real `Cursor.exe` path and `args = ["<Cursor cli.js>", "--trust"]`. Set `DIREXIO_CURSOR_COMMAND`, `DIREXIO_CONNECT_AGENT_CMD`, or `DIREXIO_CONNECT_AGENT_OPTIONS_TOML` to override this default.
