...
- **S3_PROVISION**: 按 S1 记录的 provider 创建资源。无 Agent 的 legacy 部署使用 `dirextalk/message-server:latest` 并记录 `source=default_latest`；启用 Agent 的生产部署必须通过 `DIREXTALK_MESSAGE_SERVER_RELEASE_IMAGE` 提供精确的公开稳定 `dirextalk/message-server:vX.Y.Z@sha256:<digest>`，在 AWS 变更前冻结到 state，拒绝 latest、预发布以及独立的 debug-only `MESSAGE_SERVER_IMAGE` 覆盖。同时记录独立 updater 的 version/commit/SHA-256 pin。Lightsail 路径创建 Ubuntu 24.04 实例、密钥对、静态 IP 和防火墙端口；EC2 创建 Ubuntu 24.04 amd64 实例、密钥对、安全组、Elastic IP 和 50 GiB gp3 root EBS。两条路径的 launch data 都只包含一次性 64-hex 主机身份 nonce；稳定 IP 附着后先验证远端 nonce 并固定 SSH host key，再通过严格固定 host key 的 SSH 传输并执行本地冻结的完整 bootstrap。两条路径在校验宿主为 Ubuntu 22.04 或 24.04 x86_64 后，bootstrap 下载固定 updater Release URL、核对 deployer 固定 SHA-256 并原子安装。EC2 Agent 镜像必须来自同账号、同 region、固定名为 `dirextalk-agent` 的私有 ECR，并使用不可变预发布 `tag@sha256`；每次尝试在本地获取一小时最小权限 STS 会话，仅通过 SSH stdin 传送 ECR Docker password，远端用 root-only `/run/dirextalk-ecr-auth` 登录、拉取、启动、登出、删除并回读确认目录不存在。随后 S3 通过 SSH 触发固定 updater 的可续跑 reconcile，不复制 updater 二进制。AWS/STS 凭据和 registry password 不进入 user-data、`.env`、Compose、state、argv、日志、报告或仓库。
...
- **S4_BOOTSTRAP_STACK**: 等待经 nonce 验证且固定 host key 的 EC2 或 Lightsail SSH bootstrap 安装 Docker 并启动 `postgres:18 + message-server + caddy + coturn`，轮询 `https://<domain>/healthz`。
