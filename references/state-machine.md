...
- **S3_PROVISION**: 按 S1 记录的 provider 创建资源。新部署直接选择 `dirextalk/message-server:latest`，在 `state.json.server_release` 记录 `source=default_latest`、`version=latest` 和镜像引用，不访问 message-server GitHub Release；每次新部署由容器运行时拉取当时发布的 latest 镜像。同时记录独立 updater 的 version/commit/SHA-256 pin。Lightsail 路径创建 Ubuntu 24.04 实例、密钥对、静态 IP 和防火墙端口；EC2 创建 Ubuntu 24.04 amd64 实例、密钥对、安全组、Elastic IP 和 50 GiB gp3 root EBS。EC2 的 user-data 包含 bootstrap；Lightsail 只发送含一次性主机身份 nonce 的最小启动脚本，在静态 IP 附着后用该 nonce 验证首次 SSH host key，再通过已固定的 SSH host key 加密传输并执行冻结的本地 bootstrap，完整 bundle 不会进入 user-data。两条路径在校验宿主为 Ubuntu 22.04 或 24.04 x86_64 后，bootstrap 直接下载固定 updater Release URL、核对 deployer 固定 SHA-256 并原子安装。随后 S3 通过 SSH 触发固定 updater 的可续跑 reconcile，不复制 updater 二进制。`MESSAGE_SERVER_IMAGE` 只在同时设置 `DIREXTALK_ALLOW_MESSAGE_SERVER_IMAGE_OVERRIDE=1` 的 debug/legacy 部署中可用。
...
- **S4_BOOTSTRAP_STACK**: 等 EC2 cloud-init 或经验证的 Lightsail SSH bootstrap 安装 Docker 并启动 `postgres:18 + message-server + caddy + coturn`，轮询 `https://<domain>/healthz`。
