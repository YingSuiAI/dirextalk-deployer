...
- **S3_PROVISION**: 按 S1 记录的 provider 创建资源。Lightsail 路径创建 $12 Linux 实例、密钥对、静态 IP 和防火墙端口，并只使用已查询到的可用 Lightsail AZ；执行阶段不会静默切到 EC2。当 `DIREXTALK_CLOUD_PROVIDER=ec2` 或 S1 已记录 EC2 fallback 时，创建 EC2、密钥对、安全组、Elastic IP 和 50 GiB gp3 root EBS。两条路径都会按 DNS 模式处理 Route53 hosted zone/A 记录或等待外部 DNS；EC2 渲染 cloud-config user-data，Lightsail 渲染 launch shell user-data。默认镜像 `MESSAGE_SERVER_IMAGE=dirextalk/message-server:latest`。
...
- 环境文件: `~/.dirextalk/nodes/<service_id>/env`
