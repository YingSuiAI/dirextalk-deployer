# Dirextalk Connection Stack V2 — contract-only skeleton

This directory defines the V2 **security contract**, not an activated cloud
executor. It is deliberately separate from `scripts/connection-stack` (V1),
which must not be extended in place because V1 approvals do not bind the Flutter
device signature scope required by the Cloud Orchestrator.

Do not deploy this skeleton for a product workload. Its SAM template creates
retained KMS and DynamoDB scaffolding plus a tightly throttled API, but its
Lambda handler always returns `connection_stack_v2_not_activated`. It has no AWS
SDK client, no DynamoDB permissions, no EC2/EBS/IAM/PassRole/SSH capability,
and no worker bootstrap path. It cannot provision, observe, stop, start, or
...
A production DynamoDB implementation must execute the following in one
...
- New command: require the active generation and a strictly advancing counter,
  then write the immutable receipt.
...
The receipt-store implementation is intentionally not part of this skeleton,
but the fake contract store proves these cases without AWS. The Lambda handler
still does not call this adapter and remains unactivated.
...
The test includes a fake challenge store proving that a reused Flutter approval
challenge fails even when an attacker supplies a new node command id.
