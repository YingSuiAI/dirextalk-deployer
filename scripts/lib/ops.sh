...
ops_stage_current_host_integration() (
  ...
  if [ -n "$remote_release_command" ]; then
    remote_release_command+="sudo /var/dirextalk-message-server/production-ops/reconcile-production.sh; "
    remote_release_command="release_lock=/var/dirextalk-message-server/.direct-release-update.lock; sudo install -o root -g root -m 0600 /dev/null \"\$release_lock\"; sudo flock -n \"\$release_lock\" sh -c $(ops_sh_quote "$remote_release_command"); "
  fi
