#!/usr/bin/env bash
# The deployer follows the Message Server and Agent latest release channels.
# Version and revision labels are recorded for runtime probes.
...
server_release_validate_pin() {
  ...
  [ "$DIREXTALK_MESSAGE_SERVER_IMAGE" = docker.io/dirextalk/message-server:latest ] || return 1
  [ "$DIREXTALK_AGENT_IMAGE" = docker.io/dirextalk/agent:latest ] || return 1
...
server_release_state_matches_pin() {
  ...
  [ "$source" = production_split ] \
    ...
    && [ "$image" = "$DIREXTALK_MESSAGE_SERVER_IMAGE" ] \
    && [ -z "$digest" ] \
    && [ "$image_ref" = "$DIREXTALK_MESSAGE_SERVER_IMAGE" ] \
    && [ -z "$manifest_digest" ]
}
...
server_release_state_is_recorded_current() {
  ...
  [ "$source" = production_split ] \
    ...
    && [ "$image" = docker.io/dirextalk/message-server:latest ] \
    && [ -z "$digest" ] \
    && [ "$image_ref" = docker.io/dirextalk/message-server:latest ] \
    && [ -z "$manifest_digest" ]
}
...
server_release_split_state_matches_pin() {
  [ "$(state_get split_release.release_catalog_origin)" = "$DIREXTALK_RELEASE_CATALOG_ORIGIN" ] \
    ...
    && [ "$(state_get split_release.message_image)" = "$DIREXTALK_MESSAGE_SERVER_IMAGE" ] \
    ...
    && [ "$(state_get split_release.agent_image)" = "$DIREXTALK_AGENT_IMAGE" ] \
...
server_release_split_state_can_advance() {
  ...
  server_release_is_version "$message_version" \
    ...
    && [ "$message_image" = docker.io/dirextalk/message-server:latest ] \
