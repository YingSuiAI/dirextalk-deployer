...
for updater_file in install.sh bootstrap-host.sh set-desired-state.sh release.env config.json config.legacy-compose-caddy.json dirextalk-updater.service dirextalk-updater-discovery.service dirextalk-updater-discovery.timer; do
  tr -d '\r' < "$HERE/updater/$updater_file" > "$WORK/updater/$updater_file"
done
chmod 0644 "$WORK/docker-compose.yml" "$WORK/Caddyfile"
chmod 0644 "$WORK/updater/release.env" "$WORK/updater/config.json" "$WORK/updater/config.legacy-compose-caddy.json" "$WORK/updater/"*.service "$WORK/updater/"*.timer
