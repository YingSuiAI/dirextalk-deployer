...
write_state() {
  ...
  mkdir -p "$(dirname "$state")" "$service_dir/dirextalk-connect"
  : > "$service_dir/dirextalk-connect/config.toml"
  json_build object \
    region=us-east-1 \
    domain_mode=user \
    "domain=$domain" \
    ...
    'resources={"instance_id":"i-test","eip_id":"eipalloc-test","sg_id":"sg-test","key_name":"dirextalk-test"}' > "$state"
}
...
current_service="$HOME/.dirextalk/nodes/a5.dirextalk.ai"
current_state="$current_service/state.json"
current_calls="$tmp/current.calls"
write_state "$current_state" "a5.dirextalk.ai" "$current_service"
run_destroy "$current_state" "$current_calls" "$current_service/dirextalk-connect"

grep -q '^dirextalk-connect daemon status --service-name a5.dirextalk.ai$' "$current_calls" || {
  echo "destroy should query the current named daemon status" >&2
  cat "$current_calls" >&2
  exit 1
}

grep -q '^dirextalk-connect daemon stop --service-name a5.dirextalk.ai$' "$current_calls" || {
  echo "destroy should stop the daemon when daemon status WorkDir matches the current service dirextalk-connect dir" >&2
  cat "$current_calls" >&2
  exit 1
}

grep -q '^dirextalk-connect daemon uninstall --service-name a5.dirextalk.ai$' "$current_calls" || {
...
other_service="$HOME/.dirextalk/nodes/b5.dirextalk.ai"
other_state="$other_service/state.json"
active_other_service="$HOME/.dirextalk/nodes/active-other"
other_calls="$tmp/other.calls"
mkdir -p "$active_other_service/dirextalk-connect"
write_state "$other_state" "b5.dirextalk.ai" "$other_service"
run_destroy "$other_state" "$other_calls" "$active_other_service/dirextalk-connect"

grep -q '^dirextalk-connect daemon status --service-name b5.dirextalk.ai$' "$other_calls" || {
  echo "destroy should query the named daemon for the service being destroyed" >&2
  cat "$other_calls" >&2
  exit 1
}

if grep -q '^dirextalk-connect daemon stop' "$other_calls"; then
  echo "destroy must not stop a daemon whose status WorkDir belongs to a different service" >&2
  cat "$other_calls" >&2
  exit 1
fi

if grep -q '^dirextalk-connect daemon uninstall' "$other_calls"; then
