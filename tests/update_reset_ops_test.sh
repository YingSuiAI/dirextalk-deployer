...
assert_contains "$update_calls" 'DIREXIO_DIR=/var/direxio-message-server'
assert_contains "$update_calls" '\[ -f /opt/p2p/docker-compose\.yml \]'
assert_contains "$update_calls" 'cd "\$DIREXIO_DIR"'
assert_contains "$update_calls" 'INIT_TOKENS="\$DIREXIO_DIR/init-tokens\.sh"'
assert_contains "$update_calls" 'bash "\$INIT_TOKENS"'
assert_contains "$update_calls" '/var/direxio-message-server/p2p/bootstrap\.json'
assert_contains "$update_calls" 'BOOTSTRAP_FILE=/opt/p2p/bootstrap\.json'
...
assert_contains "$reset_calls" 'DIREXIO_DIR=/var/direxio-message-server'
assert_contains "$reset_calls" '\[ -f /opt/p2p/docker-compose\.yml \]'
assert_contains "$reset_calls" 'cd "\$DIREXIO_DIR"'
assert_contains "$reset_calls" 'INIT_TOKENS="\$DIREXIO_DIR/init-tokens\.sh"'
assert_contains "$reset_calls" 'bash "\$INIT_TOKENS"'
assert_contains "$reset_calls" '/var/direxio-message-server/p2p/bootstrap\.json'
assert_contains "$reset_calls" '/opt/p2p/bootstrap\.json'
