...
render_controlled_public() {
  local target=$1 worker_source_cidr=${2:-10.30.0.0/24}
  ...
  DIREXTALK_WORKER_EDGE_SOURCE_CIDR="$worker_source_cidr" \
  DIREXTALK_WORKER_EDGE_EVIDENCE_FILE="$controlled_evidence" \
    bash "$renderer" "$target"
}
...
grep -Fq '# evidence_schema=dirextalk-worker-edge-evidence-v3' "$output"
grep -Fq '# worker_source_cidr=10.30.0.0/24' "$output"
! grep -Eq '__DIREXTALK_[A-Z0-9_]+__' "$output"

if render_controlled_public "$TEST_TMP/public-source.cfg" 203.0.113.0/24 \
  >"$TEST_TMP/public-source.stdout" 2>"$TEST_TMP/public-source.stderr"; then
  echo 'renderer accepted a public Worker source CIDR' >&2
  exit 1
fi
grep -Fq 'must be a private RFC1918 subnet' "$TEST_TMP/public-source.stderr"
...
if DIREXTALK_WORKER_CONTROL_DOMAIN=worker-control.example.test \
  ...
  DIREXTALK_WORKER_EDGE_ROUTE_MODE=controlled-public \
  DIREXTALK_WORKER_EDGE_LISTEN_IP=10.30.0.20 \
  DIREXTALK_WORKER_EDGE_SOURCE_CIDR=10.30.0.0/24 \
  DIREXTALK_WORKER_EDGE_EVIDENCE_FILE="$controlled_evidence" \
...
JSON
    ;;
  *' lightsail get-instance '*) printf '%s\n' '{"instance":{"name":"dirextalk-s2","arn":"arn:aws:lightsail:ap-east-1:123456789012:Instance/00000000-0000-4000-8000-000000000001","supportCode":"999999999999/i-0123456789abcdef0","location":{"regionName":"ap-east-1"},"resourceType":"Instance","privateIpAddress":"10.20.0.10","publicIpAddress":"43.199.101.138","state":{"name":"running"}}}' ;;
  ...
  *' lightsail get-instance-port-states '*)
    ...
    ;;
  *' route53 get-hosted-zone '*ZPRIVATE123*) printf '%s\n' '{"HostedZone":{"Id":"/hostedzone/ZPRIVATE123","Config":{"PrivateZone":true}},"VPCs":[{"VPCRegion":"ap-northeast-1","VPCId":"vpc-1123456789abcdef0"}]}' ;;
  *' route53 get-hosted-zone '*ZPUBLIC123*) printf '%s\n' '{"HostedZone":{"Id":"/hostedzone/ZPUBLIC123","Config":{"PrivateZone":false}}}' ;;
  ...
  *' ec2 describe-security-groups '*sg-1123456789abcdef0*)
    if [ "${AWS_MOCK_EDGE_SG_READ_FAILURE:-false}" = true ]; then
      printf '%s\n' 'simulated edge security group infrastructure failure' >&2
      exit 73
    fi
    if [ "${AWS_MOCK_BAD_EDGE_SOURCE:-false}" = true ]; then worker_source=10.31.0.0/24; else worker_source=10.30.0.0/24; fi
    node - "$worker_source" <<'NODE'
const source = process.argv[2];
process.stdout.write(JSON.stringify({SecurityGroups:[{GroupId:'sg-1123456789abcdef0',OwnerId:'123456789012',VpcId:'vpc-1123456789abcdef0',IpPermissions:[{IpProtocol:'-1',IpRanges:[{Description:'dirextalk-worker-edge-v3',CidrIp:source}],Ipv6Ranges:[],PrefixListIds:[],UserIdGroupPairs:[]}],IpPermissionsEgress:[['tcp',53,'10.30.0.2/32'],['udp',53,'10.30.0.2/32'],['tcp',443,'0.0.0.0/0'],['tcp',10443,'43.199.101.138/32'],['tcp',11443,'43.199.101.138/32']].map(([IpProtocol,FromPort,CidrIp])=>({IpProtocol,FromPort,ToPort:FromPort,IpRanges:[{Description:'dirextalk-worker-edge-v3',CidrIp}],Ipv6Ranges:[],PrefixListIds:[],UserIdGroupPairs:[]})),Tags:[{Key:'dirextalk-run-id',Value:'run-worker-edge'},{Key:'dirextalk-owner',Value:'@owner:example.test'},{Key:'dirextalk-generation',Value:'7'}]}]})+'\n');
NODE
    ;;
...
DIREXTALK_WORKER_EDGE_S2_SUPPORT_CODE=999999999999/i-0123456789abcdef0 \
DIREXTALK_WORKER_EDGE_S2_PRIVATE_IP=10.20.0.10 \
DIREXTALK_WORKER_EDGE_S2_PUBLIC_IP=43.199.101.138 \
...
json_check "$reader_evidence" "data.schema === 'dirextalk-worker-edge-evidence-v3' && data.account_generation === 7 && data.edge.public_ip === '18.180.1.2' && JSON.stringify(data.security.edge_ingress) === JSON.stringify({source_cidr:'10.30.0.0/24',ip_protocol:'-1'}) && data.lightsail.control_ingress.source === '18.180.1.2/32'"
...
DIREXTALK_WORKER_EDGE_SOURCE_CIDR=10.30.0.0/24 DIREXTALK_WORKER_EDGE_SECURITY_GROUP_ID=sg-1123456789abcdef0 \
...
if PATH="$TEST_TMP/bin:$PATH" AWS_MOCK_BAD_EDGE_SOURCE=true \
DIREXTALK_WORKER_EDGE_RUN_ID=run-worker-edge DIREXTALK_WORKER_EDGE_ACCOUNT_ID=123456789012 \
DIREXTALK_WORKER_EDGE_OWNER_ID=@owner:example.test DIREXTALK_WORKER_EDGE_ACCOUNT_GENERATION=7 \
DIREXTALK_WORKER_EDGE_REGION=ap-northeast-1 DIREXTALK_WORKER_EDGE_ROUTE_MODE=controlled-public \
DIREXTALK_WORKER_EDGE_S2_REGION=ap-east-1 DIREXTALK_WORKER_EDGE_S2_INSTANCE_NAME=dirextalk-s2 \
...
DIREXTALK_WORKER_EDGE_DNS_RESOLVER_CIDR=10.30.0.2/32 DIREXTALK_WORKER_CONTROL_DOMAIN=worker-control.example.test \
DIREXTALK_MODEL_RELAY_DOMAIN=model-relay.example.test DIREXTALK_OUTBOUND_PROXY_DOMAIN=worker-proxy.example.test \
  bash "$reader" "$TEST_TMP/bad-edge-source-evidence.json" >/dev/null 2>&1; then
  echo 'evidence reader accepted an edge ingress source outside the explicit Worker source CIDR' >&2
  exit 1
fi

if PATH="$TEST_TMP/bin:$PATH" AWS_MOCK_EDGE_SG_READ_FAILURE=true \
DIREXTALK_WORKER_EDGE_RUN_ID=run-worker-edge DIREXTALK_WORKER_EDGE_ACCOUNT_ID=123456789012 \
DIREXTALK_WORKER_EDGE_OWNER_ID=@owner:example.test DIREXTALK_WORKER_EDGE_ACCOUNT_GENERATION=7 \
DIREXTALK_WORKER_EDGE_REGION=ap-northeast-1 DIREXTALK_WORKER_EDGE_ROUTE_MODE=controlled-public \
DIREXTALK_WORKER_EDGE_S2_REGION=ap-east-1 DIREXTALK_WORKER_EDGE_S2_INSTANCE_NAME=dirextalk-s2 \
...
DIREXTALK_WORKER_EDGE_DNS_RESOLVER_CIDR=10.30.0.2/32 DIREXTALK_WORKER_CONTROL_DOMAIN=worker-control.example.test \
DIREXTALK_MODEL_RELAY_DOMAIN=model-relay.example.test DIREXTALK_OUTBOUND_PROXY_DOMAIN=worker-proxy.example.test \
  bash "$reader" "$TEST_TMP/edge-sg-read-failure-evidence.json" \
    >"$TEST_TMP/edge-sg-read-failure.stdout" 2>"$TEST_TMP/edge-sg-read-failure.stderr"; then
  echo 'evidence reader accepted an edge security group infrastructure failure' >&2
  exit 1
fi
grep -Fq 'read worker edge evidence: edge security group read failed' "$TEST_TMP/edge-sg-read-failure.stderr"
if grep -Fq 'edge security group rules or ownership tags differ' "$TEST_TMP/edge-sg-read-failure.stderr"; then
  echo 'evidence reader classified an AWS CLI failure as a topology mismatch' >&2
  exit 1
fi
